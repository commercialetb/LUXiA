...
st.set_page_config(page_title="LUXiA Ultimate Titan v10.7", layout="wide")
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS rooms 
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, project_id INTEGER, r_name TEXT, area REAL)''')
    
    # FIX: Migrazione automatica per colonne mancanti (Previene DatabaseError)
    columns_to_add = [
...
def main():
    ...
    if not st.session_state.logged_in:
        st.title("🏛️ LUXiA Ultimate Titan v10.7")
        ...
        with tab2:
            ...
            ns = st.text_input("Nome Studio"); nl = st.file_uploader("Logo", type=['png', 'jpg'])
    ...
    if not st.session_state.current_id:
        ...
        t1, t2, t3 = st.tabs(["📐 Vision & Zoom", "💡 Progettazione", "📄 Report"])

        with t1:
            col1, col2 = st.columns([1, 3])
            with col1:
                pdf = st.file_uploader("Planimetria PDF", type=['pdf'])
                if pdf or st.button("Aggiorna"):
                ...
                for i, r in enumerate(st.session_state.rooms):
                    st.session_state.rooms[i]['name'] = st.text_input(f"Vano {i}", r['name'], key=f"v_{i}")
            ...
            with col2:
                if st.session_state.img_b64:
                    ...
                    st_canvas(initial_drawing={"objects": rects}, background_color="rgba(0,0,0,0)", height=st.session_state.canv_h, width=st.session_state.canv_w, key="titan_canvas")
            ...
            for i, r in enumerate(st.session_state.rooms):
                with st.expander(f"⚙️ {r['name']} ({r['area']} mq)"):
                    ...
                    with c_i:
                        ...
                        pr = st.number_input("Prezzo €", 10, 5000, 150, key=f"pr_{i}")
                        if st.button(f"Calcola", key=f"gen_{i}"):
        ...
        with t3:
            ...
            conn = sqlite3.connect('luxia_titan.db')
            # Query sicura: ora le colonne esistono grazie a init_db()
            ...
            if not df.empty:
                st.table(df)
                st.metric("Totale Fornitura", f"€ {df['price'].sum():,.2f}")
