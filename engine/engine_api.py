...
def _load_default_luminaire_from_env():
    """Preload luminaire photometries.
    Priority:
      1) shipped presets in engine/assets/presets/*.ies (3 default keys)
      2) shipped engine/assets/default.ies as 'default'
      3) optional override via ENV LUMINAIRE_IES_BASE64 (replaces 'default')
    """
    here = os.path.dirname(__file__)

    # 1) Presets (always try)
    presets_dir = os.path.join(here, "assets", "presets")
    if os.path.isdir(presets_dir):
        for fn in sorted(os.listdir(presets_dir)):
            if not fn.lower().endswith((".ies", ".ldt")):
                continue
            ...
            try:
                ...
                _LUMINAIRE_LIB[key] = phot
            except Exception:
                continue

    # 2) Default shipped IES
    try:
        p = os.path.join(here, "assets", "default.ies")
        if os.path.exists(p) and "default" not in _LUMINAIRE_LIB:
            raw = open(p, "rb").read()
            _LUMINAIRE_LIB["default"] = _parse_ies_lm63(raw)
    except Exception:
        pass

    # 3) Optional ENV override for 'default'
...
def _scale_photometry(phot: Dict[str, Any], factor: float) -> Dict[str, Any]:
    """Scale lumens and candela distribution linearly (simple dimming model)."""
    try:
        f = float(factor)
    except Exception:
        f = 1.0
    ...
    p = dict(phot or {})
    if "lumens" in p and isinstance(p["lumens"], (int, float)):
        p["lumens"] = float(p["lumens"]) * f
    cd = p.get("cd")
    if isinstance(cd, dict):
        p["cd"] = {k: (float(v) * f if isinstance(v, (int, float)) else v) for k, v in cd.items()}
    return p
...
# ==========================================================
# LuxIA PRO - Interactive missing inputs (ask user when not inferable)
# ==========================================================
def _collect_missing_inputs(opts: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Return questions for parameters we cannot reliably infer.
    We still run with defaults, but we surface questions so the UI can ask the user and re-run.
    """
    missing: List[Dict[str, Any]] = []
    def need(key: str, question: str, default: Any):
        if key in opts and opts.get(key) not in (None, ""):
            return
        missing.append({"key": key, "question": question, "default": default})

    # Heights are critical for correct results (UNI / UGR / vertical)
    need("room_height_m", "Altezza ambiente (m) — se diversa dal default", float(os.getenv("ROOM_HEIGHT_M", "2.7")))
    need("luminaire_mount_z_m", "Quota installazione apparecchi (m da pavimento)", float(os.getenv("LUMINAIRE_MOUNT_Z_M", str(ROOM_HEIGHT_M_DEFAULT))))
    # Optional but useful
    if bool(opts.get("daylight", False)) or bool(opts.get("calc_daylight", False)):
        need("window_head_height_m", "Quota architrave finestre (m) — solo per daylight", float(os.getenv("WINDOW_HEAD_HEIGHT_M", "2.2")))
        need("window_sill_height_m", "Quota davanzale finestre (m) — solo per daylight", float(os.getenv("WINDOW_SILL_HEIGHT_M", "0.9")))
    return missing

def _apply_user_inputs_as_env_defaults(opts: Dict[str, Any]) -> None:
    """Map UI keys to existing engine ENV/opts usage."""
    if "room_height_m" in opts and opts.get("room_height_m") not in (None, ""):
        try:
            os.environ["ROOM_HEIGHT_M"] = str(float(opts["room_height_m"]))
        except Exception:
            pass
    if "luminaire_mount_z_m" in opts and opts.get("luminaire_mount_z_m") not in (None, ""):
        try:
            os.environ["LUMINAIRE_MOUNT_Z_M"] = str(float(opts["luminaire_mount_z_m"]))
        except Exception:
            pass
...
def _extract_from_dxf_real(dxf_bytes: bytes, raster_png: bytes, opts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Units inference (robust): header INSUNITS -> extents heuristic -> optional override
    meters_per_unit_override = float(opts.get("meters_per_unit_override") or 0.0) if isinstance(opts, dict) else 0.0
    if meters_per_unit_override and meters_per_unit_override > 0:
        unit_info = {"meters_per_unit": float(meters_per_unit_override), "confidence": 1.0, "hint": "override"}
    else:
        unit_info = _infer_dxf_units(doc, segs)
    unit_to_m = float(unit_info.get("meters_per_unit") or 1.0)

    # derive a friendly label
    units_guess = "m"
    if abs(unit_to_m - 0.001) < 1e-9:
        units_guess = "mm"
    elif abs(unit_to_m - 0.01) < 1e-9:
        units_guess = "cm"
    elif abs(unit_to_m - 0.3048) < 1e-6:
        units_guess = "ft"
    elif abs(unit_to_m - 0.0254) < 1e-6:
        units_guess = "in"

    # fallback max dimension for raster sizing later
    try:
        maxdim = float(maxx - minx) if float(maxx - minx) > float(maxy - miny) else float(maxy - miny)
    except Exception:
        maxdim = 1.0
...
def _apply_lighting_intent(extracted: Dict[str, Any], opts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    This MUST never break the pipeline: if anything goes wrong, caller should catch exceptions.
    """
    if not isinstance(extracted, dict):
        return extracted
    rooms = extracted.get("rooms") or []
    ...
    # rhythm / module (if a CAD grid was detected, snap spacing to 3/5/7 modules)
    modules = list(opts.get("rhythm_modules") or [3, 5, 7])
    base_module = float(((extracted.get("meta") or {}).get("cad_grid") or {}).get("spacing_m") or 0.0)
    ...
    for room in rooms:
        ...
        # Snap perimeter spacing to rhythm if grid exists
        try:
            if base_module > 0 and float(prof.get("perimeter_spacing_m") or 0) > 0:
                prof["perimeter_spacing_m"] = _rhythm_step(float(prof.get("perimeter_spacing_m")), base_module, modules)
        except Exception:
            pass

        # -----------------------------
        # Perimeter wash / vertical
        # -----------------------------
        if bool(prof.get("perimeter_wash")):
            ...
            if wall_seg:
                ...
                pts = _points_along_segment(wp1, wp2, float(prof.get("perimeter_spacing_m") or 3.8))
                off = float(prof.get("perimeter_offset_m") or 0.55)
                pts = [(x + nx * off, y + ny * off) for (x, y) in pts if _pip(x + nx * off, y + ny * off, poly)]
            else:
                pts = _perimeter_points(
                    poly,
                    float(prof.get("perimeter_offset_m") or 0.55),
                    float(prof.get("perimeter_spacing_m") or 3.8),
                )
                ...
                added.append(
                    {
                        ...
                        "lumens": int(base_lm * float(prof.get("perimeter_lumens_factor") or 0.60)),
                        "watt": float(base_w * float(prof.get("perimeter_lumens_factor") or 0.60)),
                        "cct_k": cct,
                        "cri": cri,
                        "beam_deg": 90,
                        "placement_rule": "PERIMETER",
                        "spacing_m": float(prof.get("perimeter_spacing_m") or 3.8),
                        "offset_wall_m": float(prof.get("perimeter_offset_m") or 0.55),
        ...
        if bool(prof.get("accent")) and int(apol.get("accent_count") or 0) > 0:
            pts = _feature_points(room, int(apol.get("accent_count") or 0))
            for j, (x, y) in enumerate(pts):
                added.append(
                    {
                        ...
                        "fixture_type": "SPOT",
                        "layer": "ACCENT",
                        "lumens": int(base_lm * float(prof.get("accent_lumens_factor") or 0.55)),
                        "watt": float(base_w * float(prof.get("accent_lumens_factor") or 0.55)),
                        "cct_k": cct,
                        "cri": cri,
                        "beam_deg": int(apol.get("beam_deg") or prof.get("accent_beam_deg") or 36),
        ...
        # -----------------------------
        # Vertical emphasis gate (optional)
        # Adds extra wallwash points only if vertical illuminance is low.
        # -----------------------------
        if bool(prof.get("vertical_emphasis")):
            ...
            if ev and ev_target > 0 and float(ev.get("Ev_avg") or 0.0) < ev_target and wall_seg2:
                (wp1, wp2) = wall_seg2
                nx, ny = _segment_normal_inward(poly, wp1, wp2)
                step = max(1.6, float(prof.get("perimeter_spacing_m") or 3.8))
                pts2 = _points_along_segment(wp1, wp2, step)
                off = float(prof.get("perimeter_offset_m") or 0.55)
                ...
                try:
                    room["vertical_ev"] = ev
                    room["vertical_ev_target"] = float(ev_target)
                except Exception:
                    pass
...
def _autofix_uni_to_ok(extracted: Dict[str, Any],
    ...
    """
    if not isinstance(extracted, dict):
        goal = str(opts.get("goal") or "balanced").lower()
        # Post-OK optimization: reduce power or quantity while keeping UNI OK (best-effort)
        if goal in ("min_qty", "min_power"):
            # 1) try dimming (min_power) by scaling lumens/candela
            if goal == "min_power":
                best_factor = 1.0
                for f in (0.9, 0.8, 0.7, 0.6):
                    phot_s = _scale_photometry(phot, f)
                    ok_here = True
                    for room in extracted.get("rooms", []):
                        targets = _get_uni_targets(room)
                        m2 = _calc_room_grid_metrics(room, extracted.get("lights", []), phot_s)
                        if _status_vs_targets(m2, targets, _ugr_approx(room, phot_s)).get("status") != "OK":
                            ok_here = False
                            break
                    if ok_here:
                        best_factor = f
                        ...
                        break
                if best_factor < 1.0:
                    extracted.setdefault("meta", {})["dim_factor"] = best_factor
                    if hasattr(checks, "append"):
                        checks.append({"code":"OPT_DIM", "detail": f"Dimming applicato: {best_factor:.2f} (goal=min_power)"})
                    # keep using scaled phot for any later calculation in this call
                    phot = _scale_photometry(phot, best_factor)

            # 2) try reduce quantity (min_qty or fallback) by removing a subset per room
            if goal == "min_qty" or goal == "min_power":
                # group lights per room using point-in-polygon test
                new_lights = list(extracted.get("lights", []))
                changed = False
                for room in extracted.get("rooms", []):
                    poly = room.get("polygon_m") or []
                    if len(poly) < 3:
                        continue
                    inside_idx = [i for i,lx in enumerate(new_lights) if _point_in_poly((lx.get("x_m"), lx.get("y_m")), poly)]
                    if len(inside_idx) <= 2:
                        continue
                    # remove every 3rd light, test UNI, keep if OK
                    trial = [l for k,l in enumerate(new_lights) if k not in inside_idx[::3]]
                    ok_here = True
                    for rr in extracted.get("rooms", []):
                        targets = _get_uni_targets(rr)
                        m2 = _calc_room_grid_metrics(rr, trial, phot)
                        if _status_vs_targets(m2, targets, _ugr_approx(rr, phot)).get("status") != "OK":
                            ok_here = False
                            break
                    if ok_here:
                        new_lights = trial
                        changed = True
                if changed:
                    extracted["lights"] = new_lights
                    if hasattr(checks, "append"):
                        checks.append({"code":"OPT_QTY", "detail": "Riduzione quantità luci mantenendo UNI OK (best-effort)"})
        return extracted
    rooms = extracted.get("rooms") or []
    if not rooms:
        return extracted
    ...
    for room in rooms:
        ...
        for _ in range(max_iters):
            ...
            try:
                df = float(extracted.get("meta", {}).get("dim_factor") or 1.0)
                if df and df != 1.0:
                    phot_room = _scale_photometry(phot_room, df)
            except Exception:
                pass
            dims = _room_bbox_dims_m(room, None)
            H = float(room.get("height_m") or ROOM_HEIGHT_M_DEFAULT)
...
async def planimetry_analyze(
    ...
    """Accepts PDF/JPG/PNG/DXF/DWG and returns extracted geometry + verification + lux analysis.

    Options (JSON string):
      - autonomy_level: 0|1|2 (default 1)
      - autofix_uni: bool (default true when autonomy>=1)
      - designer_mode: bool (default true when autonomy>=1)
      - daylight_mode: bool (default false)  # calculated ONLY if requested
      - lighting_mode: string (designer profile)
      - goal: "min_power"|"min_qty"|"balanced" (default balanced)
      - write_lights_dxf: bool (default true for DXF)
      - write_legend: bool (default true)
      - meters_per_unit_override: float (optional)
      - constraints: { "no_lights_polygons_m": [ [ [x,y],... ], ... ] }
    """
    ...
    # Apply interactive user inputs (heights, etc.) if provided
    _apply_user_inputs_as_env_defaults(_opts)

    autonomy = int(_opts.get("autonomy_level", 1) or 1)
    autonomy = max(0, min(2, autonomy))

    # Default behavior: autonomy=1 means LuxIA decides & fixes; autonomy=0 means analysis-only
    if autonomy >= 1:
        _opts.setdefault("autofix_uni", True)
        _opts.setdefault("autofix_uni_post_intent", True)
        _opts.setdefault("designer_mode", True)
        _opts.setdefault("align_to_axis", True)
        _opts.setdefault("write_lights_dxf", True)
        _opts.setdefault("write_legend", True)
    else:
        _opts.setdefault("autofix_uni", False)
        _opts.setdefault("autofix_uni_post_intent", False)
        _opts.setdefault("designer_mode", False)
        _opts.setdefault("align_to_axis", False)

    _opts.setdefault("daylight_mode", False)  # MUST be explicit to run
    _opts.setdefault("goal", "balanced")
    ...
    # Deterministic checks (hard gate)
    checks_dict = _deterministic_checks(extracted)
    checks_list: List[Dict[str, Any]] = []
    if checks_dict.get("issues"):
        for it in checks_dict["issues"]:
            checks_list.append({"code": "CHECK", "detail": str(it)})
    if checks_dict.get("warnings"):
        for it in checks_dict["warnings"]:
            checks_list.append({"code": "WARN", "detail": str(it)})

    # Overlay + Vision verifier ("prova del nove")
    overlay_png = None
    try:
    ...
    try:
        ...
        vres = {"enabled": vision.enabled, "provider": None, "pass": False, "confidence": 0.0, "issues": [{"code":"VISION_ERR","detail": str(e)}], "suggested_fixes": [], "observations": {}}
    ...
    # -------------------------
    # Lux calculation + UNI autofix (grid + UGR approx/tables)
    # -------------------------
    if isinstance(extracted, dict):
        extracted.setdefault("meta", {})
        extracted["meta"]["autonomy_level"] = autonomy
        extracted["meta"]["goal"] = str(_opts.get("goal") or "balanced")
        extracted["meta"]["daylight_mode"] = bool(_opts.get("daylight_mode"))
        extracted["meta"]["decision_trace"] = []

    def trace(step: str, detail: str):
        try:
            extracted["meta"]["decision_trace"].append({"step": step, "detail": detail, "ts": int(time.time())})
        except Exception:
            pass

    if bool(_opts.get("autofix_uni", False)) and isinstance(extracted, dict) and extracted.get("rooms") and extracted.get("lights"):
        try:
            trace("autofix_uni", "Avvio ottimizzazione per raggiungere UNI (lux/U0/UGR) con obiettivo: " + str(_opts.get("goal")))
            extracted = _autofix_uni_to_ok(extracted, _opts, phot, checks_list)
        except Exception as e:
            checks_list.append({"code":"AUTOFIX_ERR", "detail": str(e)})

    if bool(_opts.get("designer_mode", False)) and isinstance(extracted, dict) and extracted.get("rooms") and extracted.get("lights"):
        try:
            trace("designer_mode", "Applico layer designer controllato (perimeter/vertical/accent) + ritmo griglia CAD")
            extracted = _apply_lighting_intent(extracted, _opts)
            extracted = _align_room_lights_to_axis(extracted, _opts)
        except Exception as e:
            checks_list.append({"code":"DESIGNER_ERR", "detail": str(e)})

    if bool(_opts.get("autofix_uni_post_intent", False)) and isinstance(extracted, dict) and extracted.get("rooms") and extracted.get("lights"):
        try:
            trace("autofix_post_intent", "Ri-ottimizzazione post-layer designer per mantenere UNI OK")
            extracted = _autofix_uni_to_ok(extracted, _opts, phot, checks_list)
        except Exception as e:
            checks_list.append({"code":"AUTOFIX2_ERR", "detail": str(e)})

    # Lux results per-room
    if isinstance(extracted, dict) and extracted.get("rooms") and extracted.get("lights"):
        ...
        for room in extracted.get("rooms", []):
            ...
            lux_results.append({
                ...
                "ugr_approx": ugr,
                "vertical": ev,
                "luminaire_key": sel_key,
            })
        extracted["lux_results"] = lux_results

    # Daylight (separato): only if requested
    if bool(_opts.get("daylight_mode", False)) and isinstance(extracted, dict) and extracted.get("rooms"):
        try:
            trace("daylight_mode", "Calcolo daylight semplificato (solo su richiesta)")
            extracted["daylight_results"] = _calc_daylight_stub(extracted, _opts)
        except Exception as e:
            checks_list.append({"code":"DAYLIGHT_ERR", "detail": str(e)})


    # Interactive questions for missing parameters (heights, etc.)
    needs_user_input = []
    try:
        needs_user_input = _collect_missing_inputs(_opts)
        extracted.setdefault("meta", {})["needs_user_input"] = needs_user_input
    except Exception:
        needs_user_input = []
    ...
    return {
        ...
        "extracted": extracted,
        "checks": {"ok": bool(checks_dict.get("ok")), "confidence": float(checks_dict.get("confidence") or 0.0), "items": checks_list},
        "vision": vres,
...
@app.get("/luminaires/presets")
async def luminaires_presets():
    """Return bundled preset keys (shipped with the engine) + currently loaded library keys."""
    presets = []
    try:
        here = os.path.dirname(__file__)
        pdir = os.path.join(here, "assets", "presets")
        if os.path.isdir(pdir):
            for fn in sorted(os.listdir(pdir)):
                if fn.lower().endswith((".ies", ".ldt")):
                    presets.append(os.path.splitext(fn)[0].strip().lower())
    except Exception:
        presets = []
    keys = sorted(set(list(_LUMINAIRE_LIB.keys()) + presets))
    return {"ok": True, "presets": presets, "keys": keys}
...
# ==========================================================
# Simple Chat Assistant (UI chatbot)
# ==========================================================
...
@app.post("/assistant/chat", response_model=ChatResp)
async def assistant_chat(req: ChatReq):
    # Keep it lightweight and safe: short, practical, asks only missing info.
    system = (
        "Sei LuxIA, assistente tecnico per lighting design e gestione progetto. "
        "Rispondi in italiano, in modo operativo e sintetico. "
        "Se mancano dati indispensabili (es. altezze, destinazione d'uso, target UNI), chiedili in modo puntuale. "
        "Evita calcoli daylight se non richiesti esplicitamente."
    )

    # Build context summary (small)
    ctx = req.context or {}
    meta = (ctx.get("meta") or {})
    areas = ctx.get("areas") or []
    ctx_txt = ""
    if areas:
        names = [a.get("name") or a.get("id") for a in areas][:8]
        ctx_txt += f"Aree: {', '.join([str(n) for n in names if n])}. "
    if meta.get("needs_user_input"):
        q = meta.get("needs_user_input")
        keys = [x.get("key") for x in q if x.get("key")]
        if keys:
            ctx_txt += f"Dati mancanti: {', '.join(keys)}. "

    # Assemble messages
    messages = [{"role": "system", "content": system}]
    if ctx_txt:
        messages.append({"role": "system", "content": "Contesto progetto: " + ctx_txt})
    for m in (req.history or [])[-10:]:
        role = m.get("role")
        if role in ("user", "assistant") and m.get("content"):
            messages.append({"role": role, "content": str(m["content"])[:3000]})
    messages.append({"role": "user", "content": str(req.message)[:4000]})

    try:
        out = llm.chat(messages)
        assistant = out.get("text") if isinstance(out, dict) else str(out)
    except Exception as e:
        assistant = f"Errore chat: {e}"

    new_hist = (req.history or []) + [{"role": "user", "content": req.message}, {"role": "assistant", "content": assistant}]
    return {"ok": True, "assistant": assistant, "history": new_hist[-20:]}
