...
# -----------------------------------------------------------------------------
# DATABASE LAMPADE + REQUISITI UNI EN 12464-1
# -----------------------------------------------------------------------------
DB_LAMPADE = {
...
# -----------------------------------------------------------------------------
# FUNZIONI CALCOLO ILLUMINOTECNICO
# -----------------------------------------------------------------------------
def calcola_area(area: dict) -> dict:
    """Metodo del flusso luminoso UNI EN 12464-1 semplificato."""
...
def genera_dxf(risultati: list) -> str:
    """DXF semplificato con soli punti luce."""
    out = "0\nSECTION\n2\nENTITIES\n"
    lid = 1
    for r in risultati:
        ox = r.get("offset_x", 0)
...
def genera_isolux(ax, coords, lamp, sup, alt):
    """Mappa isolux 2D semplificata su piano di lavoro."""
    lato = np.sqrt(sup)
    ...
    for (lx, ly) in coords:
        ax.plot(lx, ly, "o", color="#fbbf24", ms=7,
...
def genera_pdf(progetto: dict, risultati: list) -> BytesIO:
    """PDF A3 con tavola generale + verifiche (multi pagina)."""
    buf = BytesIO()
    with PdfPages(buf) as pdf:
        ...
        ax_h.axis("off")
        ax_h.add_patch(
            mpatches.Rectangle((0, 0), 1, 1, facecolor="#1a365d", edgecolor="none")
        )
        ax_h.text(
            0.01,
            0.62,
            "TAVOLA ILLUMINOTECNICA",
            color="white",
            fontsize=18,
            fontweight="bold",
            va="center",
        ...
        ax_p.set_title(
            "PLANIMETRIA - POSIZIONAMENTO APPARECCHI",
            fontsize=10,
            fontweight="bold",
            pad=6,
        )
        ...
        for r in risultati:
            ...
            fig2, axes = plt.subplots(
                2, 3, figsize=(42 / 2.54, 29.7 / 2.54), dpi=120
            )
            fig2.patch.set_facecolor("white")
            fig2.suptitle(
                f"VERIFICHE ILLUMINOTECNICHE - {r['nome'].upper()} | "
                f"{r['tipo_locale']} | UNI EN 12464-1:2021",
                fontsize=11,
                fontweight="bold",
                color="#1a365d",
                y=0.99,
            ...
            ax.axis("off")
            ax.set_title(
                "DATI DI CALCOLO", fontsize=9, fontweight="bold", color="#1a365d"
            )
            dati = [
                ...
                ["Pot. specifica", f"{calc['wm2']} W/m²"],
                ["UGR apparecchio", str(lamp["ugr"])],
                ["UGR ammesso", f"< {calc['ugr_max']}"],
            ]
            tb = ax.table(
                cellText=dati, loc="center", cellLoc="left", colWidths=[0.50, 0.50]
            )
            ...
            ax = axes[0, 1]
            ax.set_title(
                "MAPPA ISOLUX 2D", fontsize=9, fontweight="bold", color="#1a365d"
            )
            ...
            ax.axis("off")
            ax.set_title(
                "VERIFICA CONFORMITÀ", fontsize=9, fontweight="bold", color="#1a365d"
            )
            checks = [
                (
                    "Illuminamento medio",
                    f"{calc['E_m']} lux ≥ {calc['E_t']} lux",
                    calc["ok_lux"],
                ),
                (
                    "UGR (abbagliamento)",
                    f"{lamp['ugr']} ≤ {calc['ugr_max']}",
                    calc["ok_ugr"],
                ),
            ...
            ax.add_patch(
                mpatches.Rectangle(
                    (0.02, 0.01), 0.96, 0.12, facecolor="#22c55e", edgecolor="none"
                )
            )
            ax.text(
                0.50,
                0.07,
                "CONFORME UNI EN 12464-1:2021",
                fontsize=11,
                fontweight="bold",
                color="white",
                ha="center",
            ...
            # Profilo illuminamento
            ax = axes[1, 0]
            ax.set_title(
                "PROFILO ILLUMINAMENTO ASSE X",
                fontsize=9,
                fontweight="bold",
                color="#1a365d",
            )
            ...
            ax.set_ylabel("Illuminamento [lux]", fontsize=8)
            ...
            ax.axis("off")
            ax.set_title(
                "SCHEDA APPARECCHIO", fontsize=9, fontweight="bold", color="#1a365d"
            )
            scheda = [
            ...
            ax.axis("off")
            ax.set_title(
                "PREVENTIVO AREA", fontsize=9, fontweight="bold", color="#1a365d"
            )
            ...
            tp = ax.table(
                cellText=prows,
                colLabels=["VOCE", "IMPORTO"],
                loc="center",
                cellLoc="center",
                colWidths=[0.68, 0.32],
            )
            tp.auto_set_font_size(False)
            tp.set_fontsize(8)
...
# -----------------------------------------------------------------------------
# RENDERING 3D (NON REALTIME, MA IMMAGINE PNG FOTOREALISTICA)
# -----------------------------------------------------------------------------
def genera_rendering(area: dict, calc: dict) -> BytesIO:
    """Rendering 3D semplificato dell’area: box + scrivanie + lampade."""
    ...
    # Materiali base
    mat_pav = (0.7, 0.68, 0.65)
    mat_pareti = (0.92, 0.90, 0.88)
    mat_soff = (0.98, 0.97, 0.96)
    mat_legno = (0.3, 0.2, 0.1)
    mat_led = (1.0, 0.9, 0.6)

    # Pavimento
    fl = Poly3DCollection(
        [[(0, 0, 0), (lato, 0, 0), (lato, lato, 0), (0, lato, 0)]],
        alpha=0.95,
    )
    fl.set_facecolor(mat_pav)
    fl.set_edgecolor("#888")
    ax.add_collection3d(fl)

    # Soffitto
    ce = Poly3DCollection(
        [[(0, 0, alt), (lato, 0, alt), (lato, lato, alt), (0, lato, alt)]],
        alpha=0.8,
    )
    ce.set_facecolor(mat_soff)
    ce.set_edgecolor("#ccc")
    ...
    for w in walls:
        ...
        wl.set_facecolor(mat_pareti)
        wl.set_edgecolor("#bbb")
        ax.add_collection3d(wl)

    # Scrivanie (pattern 2x2 al centro stanza)
    for sx in [lato * 0.3, lato * 0.7]:
        for sy in [lato * 0.3, lato * 0.7]:
            desk_pts = [
                [(sx - 0.6, sy - 0.3, 0),
                 (sx + 0.6, sy - 0.3, 0),
                 (sx + 0.6, sy - 0.3, 0.75),
            ...
            desk = Poly3DCollection(desk_pts, alpha=0.95)
            desk.set_facecolor(mat_legno)
            desk.set_edgecolor("#4a3c2e")
            ax.add_collection3d(desk)
    ...
    for (lx, ly) in coords:
        ...
        for sp, alp in [(0.5, 0.18), (1.0, 0.10), (1.8, 0.05)]:
            for ang in theta[::3]:
                ax.plot(
                    [lx, lx + sp * np.cos(ang)],
                    [ly, ly + sp * np.sin(ang)],
    ...
    # Distribuzione luce pavimento
    Xg, Yg = np.meshgrid(
        np.linspace(0.1, lato - 0.1, 40), np.linspace(0.1, lato - 0.1, 40)
    )
    Zg = np.zeros_like(Xg)
    ...
    ax.plot_surface(
        Xg,
        Yg,
        np.full_like(Xg, 0.02),
        facecolors=plt.cm.YlOrRd(Zn),
        alpha=0.55,
        shade=False,
    )
    ...
    fig.text(
        0.5,
        0.97,
        f"RENDERING 3D — {area['nome'].upper()}",
        fontsize=13,
        fontweight="bold",
        color="white",
        ha="center",
...
# PREVENTIVO
# -----------------------------------------------------------------------------
def calc_preventivo(risultati: list) -> dict:
    righe, tm, ti = [], 0, 0
    for r in risultati:
        lamp = DB_LAMPADE[r["lampada"]]
...
with st.sidebar:
    ...
    plan_file = st.file_uploader(
        "Carica planimetria (PDF/PNG/JPG)", type=["pdf", "png", "jpg", "jpeg"]
    )
    ...
    use_ai = st.toggle("Abilita AI (Ollama opzionale)", value=False)
    if use_ai:
        st.info(
            "Per AI Vision serve Ollama + llava:13b in locale.\n"
            "Se non li hai, lascia disattivato."
        )
...
<div class="header-box">
  ...
  <p style="margin:.3rem 0 0;opacity:.85">
    ...
    PDF A3 Professionale • Rendering 3D • AI Vision (opzionale)
  </p>
</div>
""",
    unsafe_allow_html=True,
        ...
        for i, a in enumerate(st.session_state.aree):
            ...
            with c1:
                ...
                st.markdown(
                    ...
                    f'{a["tipo_locale"]} | {a["superficie_m2"]}m² | '
    ...
    if not st.session_state.aree:
        ...
        if "risultati" in st.session_state:
                ...
                [
                    {
                        ...
                        "W": r["calc"]["W_t"],
                        "W/m²": r["calc"]["wm2"],
                        "Lux ok": r["calc"]["ok_lux"],
                        "UGR ok": r["calc"]["ok_ugr"],
                        "Ra ok": r["calc"]["ok_ra"],
                    }
                    for r in rl
                ]
            ...
            st.download_button(
                "Scarica CSV",
                csv,
                f"calcoli_{datetime.now():%Y%m%d}.csv",
                "text/csv",
            )
...
with tab6:
    ...
    if "risultati" not in st.session_state:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            mg_sl = st.slider("Margine %", 10, 60, 35)
        with c2:
            iva_sl = st.slider("IVA %", 0, 22, 22)
        with c3:
            sg_sl = st.slider("Spese gen. %", 5, 20, 12)
...
st.markdown("---")
st.caption(
    "Lighting Agent Pro v2.0 | UNI EN 12464-1:2021 | PDF A3 | Rendering 3D | AI Vision | 2026"
)
