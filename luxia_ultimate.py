...
# --- 1. CONFIGURAZIONE & SESSIONE (FRIZZATO) ---
st.set_page_config(page_title="LUXiA Ultimate Titan v11.0", layout="wide")
...
# --- 2. DATABASE ENGINE (RICOSTRUZIONE ATOMICA) ---
def init_db():
    conn = sqlite3.connect('luxia_titan.db')
    c = conn.cursor()
    # Tabelle Utenti e Progetti
    ...
    # Controllo integrità tabella ROOMS
    try:
        c.execute("SELECT area FROM rooms LIMIT 1")
    except sqlite3.OperationalError:
        # Se la colonna 'area' manca, resettiamo la tabella rooms per allinearla allo standard Titan
        st.warning("Aggiornamento struttura Database in corso...")
    ...
    # Migrazione per altre colonne opzionali
    updates = [("brand", "TEXT"), ("model", "TEXT"), ("price", "REAL"), ("qty", "INTEGER")]
    for col, typ in updates:
        try: c.execute(f"ALTER TABLE rooms ADD COLUMN {col} {typ}")
        except: pass
...
def main():
    ...
    if not st.session_state.logged_in:
        st.title("🏛️ LUXiA Ultimate Titan v11.0")
        t1, t2 = st.tabs(["Accedi", "Registra Studio"])
        with t1:
            u = st.text_input("User", key="k_u"); p = st.text_input("Pw", type="password", key="k_p")
            if st.button("Login", key="k_b"):
                conn = sqlite3.connect('luxia_titan.db')
                ...
                if r: st.session_state.update({"logged_in": True, "username": u, "studio": r[0], "logo": r[1]}); st.rerun()
        with t2:
            nu = st.text_input("Nuovo User", key="k_nu"); np = st.text_input("Pass", type="password", key="k_np")
            ns = st.text_input("Nome Studio", key="k_ns"); nl = st.file_uploader("Logo", type=['png','jpg'], key="k_nl")
            if st.button("Registra", key="k_nb"):
                l64 = base64.b64encode(nl.read()).decode() if nl else None
                conn = sqlite3.connect('luxia_titan.db')
                conn.execute("INSERT INTO users VALUES (?,?,?,?)", (nu, hashlib.sha256(np.encode()).hexdigest(), ns, l64))
    ...
    with st.sidebar:
        ...
        ai_on = st.toggle("AI Groq", value=True, key="k_ai")
        if ai_on:
            gk = st.text_input("Groq Key", type="password", key="k_gk")
        ...
        sel = st.selectbox("Progetti", ["-- Dashboard --"] + [p[1] for p in projs], key="k_ps")
        if st.button("Apri", key="k_pb"):
            p_id = next((p[0] for p in projs if p[1] == sel), None)
            st.session_state.update({"current_id": p_id, "current_name": sel if p_id else None})
            st.rerun()
        if st.button("Logout", key="k_lo"): st.session_state.clear(); st.rerun()

    # DASHBOARD
    if not st.session_state.current_id:
        st.title("📊 Dashboard")
        with st.form("f_np"):
            pn = st.text_input("Nome Cantiere")
        ...
        tab1, tab2, tab3 = st.tabs(["📐 Vision & Mappa", "💡 Calcolo AI", "📄 Report"])

        with tab1:
            c1, c2 = st.columns([1, 3])
            with c1:
                pdf = st.file_uploader("PDF Planimetria", type=['pdf'], key="k_pdf")
                if pdf or st.button("Aggiorna", key="k_up"):
                ...
                for i, r in enumerate(st.session_state.rooms):
                    st.session_state.rooms[i]['name'] = st.text_input(f"Vano {i}", r['name'], key=f"k_vn_{i}")
            ...
            with c2:
                if st.session_state.img_b64:
                    ...
                    st_canvas(initial_drawing={"objects": rects}, background_color="rgba(0,0,0,0)", height=st.session_state.canv_h, width=st.session_state.canv_w, key=f"k_c_{st.session_state.zoom_level}")
        ...
        with tab2:
            for i, r in enumerate(st.session_state.rooms):
                with st.expander(f"⚙️ {r['name']} ({r['area']} mq)"):
                    ...
                    with ci:
                        lx = st.slider("Lux", 100, 1000, 300, key=f"k_lx_{i}")
                        br = st.selectbox("Brand", ["iGuzzini", "Artemide", "Flos", "Viabizzuno"], key=f"k_br_{i}")
                        pr = st.number_input("Prezzo €", 0, 10000, 150, key=f"k_pr_{i}")
                        if st.button(f"Calcola", key=f"k_cal_{i}"):
                            if ai_on and 'groq' in st.session_state:
                                ...
                                st.session_state.strat_results[i] = f"**Offline:** {qty} lampade {br}."
                            conn = sqlite3.connect('luxia_titan.db')
        ...
        with tab3:
            ...
            try:
                ...
                if not df.empty:
                    ...
                    st.metric("Budget Totale", f"€ {df['Prezzo_Cad'].sum():,.2f}")
                else: st.info("Esegui il calcolo nei vani per vedere i dati.")
