...
st.set_page_config(page_title="LUXiA Ultimate Titan v10.9", layout="wide")
...
# --- 2. DATABASE ENGINE (ROBUSTO & AUTO-RIPARANTE) ---
def init_db():
    ...
    # Lista colonne per evoluzione schema
    schema_updates = [
        ("brand", "TEXT"), ("model", "TEXT"), ("qty", "INTEGER"),
        ("lux_target", "INTEGER"), ("strategy", "TEXT"), ("price", "REAL")
    ]
    
    # Verifica ed esecuzione aggiunta colonne
    for col_name, col_type in schema_updates:
        try:
...
def process_pdf_full(pdf_bytes, zoom_factor):
    ...
    for cnt in contours:
        if cv2.contourArea(cnt) > (15000 * (zoom_factor/2)):
            ...
            detected.append({
                "name": label.strip() or f"Vano {len(detected)+1}",
                "x": x, "y": y, "w": w, "h": h, 
...
def main():
    ...
    if not st.session_state.logged_in:
        st.title("🏛️ LUXiA Ultimate Titan v10.9")
        ...
        with t2:
            ...
            ns = st.text_input("Nome Studio", key="r_s")
            nl = st.file_uploader("Logo Studio", type=['png', 'jpg'], key="r_l")
            if st.button("Registra Studio", key="r_b"):
    ...
    with st.sidebar:
        ...
        ai_on = st.toggle("AI Groq", value=True, key="ai_t")
        if ai_on:
            gk = st.text_input("Groq Key", type="password", key="g_k")
        ...
        sel = st.selectbox("Progetti", ["-- Dashboard --"] + [p[1] for p in projs], key="p_s")
        if st.button("Carica", key="p_b"):
            p_id = next((p[0] for p in projs if p[1] == sel), None)
            st.session_state.update({"current_id": p_id, "current_name": sel if p_id else None})
            st.rerun()
        if st.button("Logout", key="out_b"): st.session_state.clear(); st.rerun()
    ...
    if not st.session_state.current_id:
        ...
        with tab1:
            ...
            with c1:
                pdf = st.file_uploader("PDF Planimetria", type=['pdf'], key="pdf_u")
                if pdf or st.button("Aggiorna", key="upd_b"):
                ...
                for i, r in enumerate(st.session_state.rooms):
                    st.session_state.rooms[i]['name'] = st.text_input(f"Vano {i}", r['name'], key=f"v_n_{i}")
            ...
            with c2:
                if st.session_state.img_b64:
                    ...
                    st_canvas(initial_drawing={"objects": rects}, background_color="rgba(0,0,0,0)", height=st.session_state.canv_h, width=st.session_state.canv_w, key=f"c_{st.session_state.zoom_level}")
            ...
            for i, r in enumerate(st.session_state.rooms):
                with st.expander(f"⚙️ {r['name']} ({r['area']} mq)"):
                    ...
                    with ci:
                        ...
                        pr = st.number_input("Prezzo €", 0, 10000, 150, key=f"pr_{i}")
                        if st.button(f"Calcola", key=f"calc_{i}"):
        ...
        with tab3:
            ...
            try:
                conn = sqlite3.connect('luxia_titan.db')
                query = f"SELECT r_name as Vano, area as MQ, brand as Marca, lux_target as Lux, price as Prezzo_Cad FROM rooms WHERE project_id={st.session_state.current_id}"
                ...
                if not df.empty:
                    ...
                    st.info("Nessun dato salvato per questo progetto.")
